...
**Milestone trigger:** Phase 1 complete when client provides written approval of conceptual direction AND HOA early feedback has been received.
...
**Revision assumptions:** 2 rounds of client revision included (including HOA compliance). HOA-driven revisions beyond 2 rounds billed as change orders at MXN 10,000/day.

**Milestone trigger:** Phase 2 complete when client provides written approval of architectural design AND written HOA approval is received.

---

### Phase 3 — Engineering Coordination
**Deliverables (identical to standalone residential):**
...
**Deliverables (standalone residential deliverables plus):**
...
- Final HOA-compliant drawing set bearing HOA approval stamp or reference number
...
- Development contractor pre-approval verification (if development maintains a contractor list, confirmed with client before bidding phase)
...
**Addition:** Contractors must be pre-approved by development if applicable (confirm with client before Phase 5 begins).
...
- Developer internal authorization verification (some developments require developer sign-off before municipal permit application; Oficio Taller verifies this requirement with client during Phase 2)
...
- **Landscape architecture:** Excluded by default; included only if client requests as an additional service.
...
Note: HOA review adds 2–4 weeks vs. standalone residential. Durations are typical estimates. Actual timeline is set by Pablo in `project-schedule.json` based on project size, complexity, and confirmed contractor start date.
...
6. Note in `revision_assumptions` that HOA-driven revisions are billed beyond the 2 included rounds
7. Do NOT include supervision (Phase 7) unless state.json or seed data explicitly includes it
8. Verify with client whether development requires contractor pre-approval list before Phase 5
